    ...
    def visualize_validation(self, epoch, writer, num_images=4):
        ...
        try:
            ...
            for i in range(min(num_images, len(samples))):
                ...
                labels = [f"{int(l)-1}:{s:.2f}" for l, s in zip(res['labels'][keep], res['scores'][keep])]
                ...
                if len(boxes) > 0:
                    img_with_boxes = draw_bounding_boxes(img_uint8, boxes, labels=labels, colors="red", width=2)
                else:
    ...
    def fit(self, ):
        ...
        for epoch in range(self.last_epoch + 1, args.epoches):
            ...
            train_stats = train_one_epoch(
                ...
                args.clip_max_norm, print_freq=args.log_step, ema=self.ema, scaler=self.scaler, writer=writer)
